...
1. Someone in your Box organization has already created the service-app for your purpose. This means that you have been issued a JSON file which contains the JWT authentication-token. 
   ...
   If so, you need to start with the [section](#use) focused on users. 

2. You need to associate the app's service-account with a folder that a user-account can access. There are a couple of ways to do this, these are discussed in the [workflow section](#workflow).

3. You have to create a Box service-app, meaning you you have to create the JWT authentication-token. This is covered in the [app-creation section](#create)
